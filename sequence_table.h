...
namespace structures
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		if (this != &other)
		{
			...
			{
				this->list_->add(new TableItem<K, T>(*otherItem));
			}
		}
		return *this;
	}
	...
	{
		//v pripade semestralky zakomentovat, v pripade neutriedenej sekvencnej tabulky, key sa  
		//v liste nachadzat viac krat
		...
		{
			throw std::logic_error("Item sa uz nachadza v liste");
		}
	}
	...
	{
		TableItem<K, T>* deleteItem = this->findTableItem(key);
		if (deleteItem != nullptr)
		{
			this->list_->tryRemove(deleteItem);
			T data = deleteItem->accessData();
			delete deleteItem;
			return data;
	...
	inline void SequenceTable<K, T>::clear()
	{
		...
		for (TableItem<K, T>* item : (*this->list_))
		{
			delete item;
		}
		this->list_->clear();
