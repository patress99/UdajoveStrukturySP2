...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		...
		{
			throw std::exception("TreeNode<T>::getBrother: Node is root!");
		}
	}
	...
	inline void Tree<T>::LevelOrderTreeIterator::populatePath(TreeNode<T>* const current)
	{
		...
		while (!processQueue->empty())
		{
			...
			{
				TreeIterator::path_->push(processNode);
				for (int i = 0; i < processNode->degree(); i++)
				{
					processQueue->push(processNode->getSon(i));
				}
			}
		}
